...
public class MailListeners {
    ...
    public void confirmRegistration(RegistrationConfirmEvent event) {
        ...
        mailService.sendToUserAsync("email-confirmation.html", user, Map.of("confirmationUrl", confirmationUrl), event.locale());
    }
    ...
    public void resetPassword(PasswordResetEvent event) {
        ...
        mailService.sendToUserAsync("password-reset.html", event.user(), Map.of("resetUrl", resetUrl), event.locale());
    }
}
